...
function themesflat_posted_category( $layout = '' ) { 	
	if ( has_category() ) {
		echo '<div class="post-categories">'.esc_html__("In - ",'saylo');
			the_category( ', ' );
		echo '</div>';
	}	
}
...
function themesflat_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $tags_list && is_single() ) {
			$tags_links = sprintf( '<div class="tags-links"><h5>' . esc_html__( 'Tags', 'saylo' ) . '</h5>' . esc_html__( ' %1$s', 'saylo' ) . '</div>', $tags_list  );
...
<nav class="navigation posts-navigation" role="navigation">
    <h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'saylo' ); ?></h2>
    <ul class="nav-links">
        <?php
				if ( is_attachment() ) :
					...
					$prev = esc_html__( 'Published In', 'saylo' );
					$date = get_the_date();
					...
					if( is_object( $prevPost ) ){
						$prevthumbnail = get_the_post_thumbnail($prevPost->ID);
						$prev_title = get_the_title($prevPost->ID);
						$prev = esc_html__( 'Previous', 'saylo' );
						$date = get_the_date();
					...
					if( is_object( $nextPost ) ){
						...
						$next = esc_html__( 'next post', 'saylo' );
						$date = get_the_date();
