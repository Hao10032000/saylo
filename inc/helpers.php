...
function themesflat_shortcode_default_id(){
    ...
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'saylo' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        ...
        $styles = apply_filters( 'themesflat/header_styles', array(
            ...
            'header-v1' => esc_html__( 'Classic', 'saylo' ),

            'header-v2' => esc_html__( 'Header style 02', 'saylo' ),

            'header-v4' => esc_html__( 'Modern', 'saylo' ),
...
function themesflat_available_social_icons() {
    ...
    $icons = apply_filters( 'themesflat_available_icons', array(
        ...
        'twitter'        => array( 'iclass' => 'icon-saylo-twitter', 'title' => 'Twitter','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),

        'facebook'       => array( 'iclass' => 'icon-saylo-facebook', 'title' => 'Facebook','share_link'=> THEMESFLAT_PROTOCOL . '://www.facebook.com/sharer/sharer.php?u=' ),

        'chat'  => array( 'iclass' => 'icon-saylo-chat', 'title' => 'Chat','share_link'=> THEMESFLAT_PROTOCOL . '://www.chat.com/share?url=' ),

        'pinterest'      => array( 'iclass' => 'icon-saylo-pinterest', 'title' => 'Pinterest','share_link' => THEMESFLAT_PROTOCOL . '://pinterest.com/pin/create/bookmarklet/?url=' ),

        'instagram'      => array( 'iclass' => 'icon-saylo-instagram', 'title' => 'Instagram','share_link' => 'https://www.instagram.com/?url=' ),

        'youtube'        => array( 'iclass' => 'icon-saylo-youtube', 'title' => 'Youtube','share_link' =>'' ),

        'vimeo'          => array( 'iclass' => 'icon-saylo-vimeo', 'title' => 'Vimeo','share_link' =>'' ),

        'behance'        => array( 'iclass' => 'icon-saylo-behance', 'title' => 'Behance','share_link' =>'' ),

        'bitcoin'        => array( 'iclass' => 'icon-saylo-bitcoin', 'title' => 'Bitcoin','share_link' =>'' ),

        'digg'           => array( 'iclass' => 'icon-saylo-digg', 'title' => 'Digg','share_link' =>'http://digg.com/submit?url=' ),

        'skype'          => array( 'iclass' => 'icon-saylo-skype', 'title' => 'Skype','share_link' => THEMESFLAT_PROTOCOL . '://web.skype.com/share?url='),

        'slack'          => array( 'iclass' => 'icon-saylo-slack', 'title' => 'Slack','share_link' => ''),

        'spotify'        => array( 'iclass' => 'icon-saylo-spotify', 'title' => 'Spotify','share_link' => ''),

        'stack-overflow' => array( 'iclass' => 'icon-saylo-stackoverflow', 'title' => 'Stack Overflow','share_link' => ''),

        'steam'          => array( 'iclass' => 'icon-saylo-steam', 'title' => 'Steam','share_link' => ''),

        'dribble'          => array( 'iclass' => 'icon-saylo-dribbble', 'title' => 'Dribble','share_link' => ''),

        'linkedin'          => array( 'iclass' => 'icon-saylo-linkedin', 'title' => 'Linkedin','share_link' => ''),     

        'steam'          => array( 'iclass' => 'icon-saylo-paper-plane', 'title' => 'Linkedin','share_link' => ''),
...
function themesflat_menu_fallback() {
    ...
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'saylo' ) . '</a></li></ul>';
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            ...
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'saylo' ), max( $paged, $page ) );
            ...
            <ul class="unstyled">
                ...
                <li class="current">                    
                    ...
                    <a href="?lang=en" class="lang_sel_sel">
                        ...
                        <span class="languages-before-icon icon-saylo-web"></span><?php echo esc_html__("English",'saylo');?><i class="icon-saylo-arrow-down" aria-hidden="true"></i>
                    ...
                    <ul class="unstyled-child">
                       ...
                       <li class="icl-en">
                            ...
                            <a href="?lang=en" class="lang_sel_sel">
                             ...
                             <?php echo esc_html__("English",'saylo');?>
                       ...
                       <li class="icl-fr fr">
                            ...
                            <a href="?lang=fr" class="lang_sel_other">
                             ...
                             <?php echo esc_html__("French",'saylo');?>
                       ...
                       <li class="icl-ge ge">
                            ...
                            <a href="?lang=it" class="lang_sel_other">
                                ...
                                <?php echo esc_html__("German",'saylo');?>
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        ...
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'saylo' ) . '</span>

        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", "saylo" ) . '"/>

    </label>

    <button type="submit" class="search-submit"><i class="icon-saylo-search"></i></button>    
...
function themesflat_social_single() {
    ...
    if( themesflat_get_opt('show_social_share') == 1 ):
        ...
        <div class="social-share-article"><h5><?php echo esc_html__( 'Share', 'saylo' ); ?></h5>        
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        ...
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'saylo' );

        } elseif ( is_search() ) {

            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'saylo' ), get_search_query() );
        ...
        if ( is_author() ) {
            ...
            $title = sprintf( esc_html__( 'Author: %s', 'saylo' ), get_the_author() );

            rewind_posts();

        } elseif ( is_day() ) {

            $title = sprintf( esc_html__( 'Daily: %s', 'saylo' ), get_the_date() );

        } elseif ( is_month() ) {

            $title = sprintf( esc_html__( 'Monthly: %s', 'saylo' ), get_the_date( 'F Y' ) );

        } elseif ( is_year() ) {

            $title = sprintf( esc_html__( 'Yearly: %s', 'saylo' ), get_the_date( 'Y' ) );
            ...
            $title = esc_html__('Book Appointment','saylo');           
